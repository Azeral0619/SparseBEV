...
img_norm_cfg = dict(
    mean=[123.675, 116.280, 103.530], std=[58.395, 57.120, 57.375], to_rgb=True
)
...
model = dict(
    ...
    stop_prev_grad=4,
    pts_bbox_head=dict(
        num_query=1600, transformer=dict(num_levels=5, num_points=8, num_frames=15)
    ),
)
...
train_pipeline = [
    ...
    dict(
        type="LoadMultiViewImageFromMultiSweepsFutureInterleave",
        prev_sweeps_num=7,
        next_sweeps_num=7,
    ),
    dict(
