...
def main():
    ...
    if local_rank == 0:
        ...
        if cfgs.resume_from is not None:
            ...
            if os.path.exists(work_dir):  # must be an empty dir
                if (
                    input('Path "%s" already exists, overwrite it? [Y/n] ' % work_dir)
                    == "n"
                ):
                    print("Bye.")
                    exit(0)
    ...
    if cfgs.eval_config["interval"] > 0:
        if world_size > 1:
            ...
            runner.register_hook(
                EvalHook(val_loader, interval=cfgs.eval_config["interval"])
            )
    ...
    if cfgs.resume_from is not None:
        ...
        if cfgs.revise_keys is not None:
            load_checkpoint(
                model, cfgs.load_from, map_location="cpu", revise_keys=cfgs.revise_keys
            )
        else:
            load_checkpoint(
                model,
