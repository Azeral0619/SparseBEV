...
def get_cam_info(nusc, sample_data):
    ...
    sensor2global_translation = (
        sensor2ego_translation @ ego2global_rotation.T + ego2global_translation
    )
...
if __name__ == "__main__":
    ...
    if args.version == "v1.0-trainval":
        sample_infos = pickle.load(
            open(os.path.join(args.data_root, "nuscenes_infos_train.pkl"), "rb")
        )
        sample_infos = add_sweep_info(nusc, sample_infos)
        mmcv.dump(
            sample_infos, os.path.join(args.data_root, "nuscenes_infos_train_sweep.pkl")
        )

        sample_infos = pickle.load(
            open(os.path.join(args.data_root, "nuscenes_infos_val.pkl"), "rb")
        )
        sample_infos = add_sweep_info(nusc, sample_infos)
        mmcv.dump(
            sample_infos, os.path.join(args.data_root, "nuscenes_infos_val_sweep.pkl")
        )

    elif args.version == "v1.0-test":
        sample_infos = pickle.load(
            open(os.path.join(args.data_root, "nuscenes_infos_test.pkl"), "rb")
        )
        sample_infos = add_sweep_info(nusc, sample_infos)
        mmcv.dump(
            sample_infos, os.path.join(args.data_root, "nuscenes_infos_test_sweep.pkl")
        )

    elif args.version == "v1.0-mini":
        sample_infos = pickle.load(
            open(os.path.join(args.data_root, "nuscenes_infos_train_mini.pkl"), "rb")
        )
        sample_infos = add_sweep_info(nusc, sample_infos)
        mmcv.dump(
            sample_infos,
            os.path.join(args.data_root, "nuscenes_infos_train_mini_sweep.pkl"),
        )

        sample_infos = pickle.load(
            open(os.path.join(args.data_root, "nuscenes_infos_val_mini.pkl"), "rb")
        )
        sample_infos = add_sweep_info(nusc, sample_infos)
        mmcv.dump(
            sample_infos,
            os.path.join(args.data_root, "nuscenes_infos_val_mini_sweep.pkl"),
        )
