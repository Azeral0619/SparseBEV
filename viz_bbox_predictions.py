...
def convert_to_nusc_box(
    bboxes, scores=None, labels=None, names=None, score_threshold=0.3, lift_center=False
):
    results = []
...
def viz_bbox(nusc, bboxes, data_info, fig, gs):
    ...
    for cam_id, cam_type in enumerate(cam_types):
        ...
        for bbox in bboxes:
            ...
            if box_in_image(bbox, intrinsic, img_size):
                ...
                bbox.render(
                    ax, view=intrinsic, normalize=True, colors=(c, c, c), linewidth=1
                )
    ...
    nusc.explorer.render_sample_data(
        lidar_data_token, with_anns=False, ax=ax, verbose=False
    )
    ax.axis("off")
