...
def main():
    ...
    for idx, data in enumerate(val_loader):
        ...
        cls_scores = torch.load(
            "{}/cls_score_stage{}.pth".format(DUMP.out_dir, args.stage_id)
        )[0]
        ...
        for frame_id in range(args.num_frames):
            ...
            for view_id in range(args.num_views):
                ...
                for query_id in query_ids:
                    xyz = sample_points_cam[
                        0, frame_id, view_id, query_id
                    ].numpy()  # [32, 3]
                    ...
                    ax.scatter(
                        cx, cy, s=4 ** (cz + 1), alpha=0.7, color="C%d" % (query_id % 5)
                    )
