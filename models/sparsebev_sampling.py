...
def sampling_4d(
    ...
    sample_points_cam = torch.matmul(lidar2img, sample_points).squeeze(
        -1
    )  # [B, T, N, Q, GP, 4]
    ...
    sample_points_cam = torch.cat(
        [sample_points_cam, i_view[..., None].float() / (N - 1)], dim=-1
    )
